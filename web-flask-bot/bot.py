...
from telegram import (
    Update, Message, InlineKeyboardButton, InlineKeyboardMarkup, InputFile
)
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        "👋 Привет! Я ваш помощник в работе с номенклатурой и каталогами.\n"
        "🔹 Используйте команды:\n"
        ...
        "/ct [модель] [узел] – найти информацию о сборочном узле"
    )
...
# Обработка команды /ct
async def ct_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    try:
        _, model_name, node_name = update.message.text.split(maxsplit=2)
        ...
        # Найдем узел по модели
        cursor.execute("""
            SELECT n.id, n.name, n.description, n.image_url
            FROM nodes n
            WHERE n.model_id = ? AND n.name = ?
        """, (model_id, node_name))
        node_info = cursor.fetchone()

        if not node_info:
            await update.message.reply_text(f"Узел '{node_name}' в модели '{model_name}' не найден.")
            return

        node_id, node_name, description, image_url = node_info

        # Сохраняем ID текущего узла для пользователя
        user_data = context.user_data
        user_data['current_node'] = node_id

        # Отправляем изображение и описание
        await update.message.reply_photo(
            photo=image_url,
            caption=f"🔧 Узел '{node_name}' ({model_name})\n\n{description}"
        )

        await update.message.reply_text("Введите номер детали для поиска:")

    except Exception as e:
        await update.message.reply_text("Неверный формат команды. Используйте: /ct [модель] [узел]")
...
async def handle_number(update: Update, context: ContextTypes.DEFAULT_TYPE):
    try:
        ...
        user_data = context.user_data
        if 'current_node' not in user_data:
            await update.message.reply_text("Сначала выберите узел с помощью команды /ct")
            return

        node_id = user_data['current_node']

        conn = get_db_connection()
        cursor = conn.cursor()

        cursor.execute("""
            SELECT number, name, quantity
            FROM parts
            WHERE node_id = ? AND number = ?
        """, (node_id, part_number))
        part = cursor.fetchone()

        if not part:
            await update.message.reply_text(f"Деталь с номером {part_number} не найдена.")
            return

        number, name, quantity = part
...
Количество: {quantity}
        ...
        await update.message.reply_text("Ошибка при обработке номера детали.")
...
async def main():
    ...
    application.add_handler(CommandHandler("ct", ct_command))
